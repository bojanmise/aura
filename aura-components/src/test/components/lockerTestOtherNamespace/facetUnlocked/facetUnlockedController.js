({
    ...
    inspectMethodParams : function(cmp, event) {
        ...
        testUtils.assertEquals(true, args.booleanParam, "Access to non-locker in different namespace: Failed to get expected value from Boolean.");
        testUtils.assertEquals("1888-08-08", args.dateParam, "Access to non-locker in different namespace: Failed to get expected value from Date.");
        testUtils.assertEquals("1888-08-08T08:08:08.888Z", args.dateTimeParam, "Access to non-locker in different namespace: Failed to get expected value from DateTime.");
        testUtils.assertEquals(88.88, args.decimalParam, "Access to non-locker in different namespace: Failed to get expected value from Decimal.");
        testUtils.assertEquals(5/7, args.doubleParam, "Access to non-locker in different namespace: Failed to get expected value from Double.");
        testUtils.assertEquals(8888, args.integerParam, "Access to non-locker in different namespace: Failed to get expected value from Integer.");
        testUtils.assertEquals(Infinity, args.longParam, "Access to non-locker in different namespace: Failed to get expected value from Long.");
        testUtils.assertEquals("GoldenState", args.stringParam, "Access to non-locker in different namespace: Failed to get expected value from String.");
        ...
        testUtils.assertTrue(caller.toString().indexOf("SecureComponentRef") === -1, "Expected raw component in non-lockerized component.");
        testUtils.assertTrue(objectParam.arrayEntry[2].toString().indexOf("SecureComponentRef") === -1, "Expected raw component in non-lockerized component.");
        testUtils.assertTrue(listParam[4].toString().indexOf("SecureComponentRef") === -1, "Expected raw component in non-lockerized component.");
        testUtils.assertTrue(mapParam.c.toString().indexOf("SecureComponentRef") === -1, "Expected raw component in non-lockerized component.");
        testUtils.assertTrue(setParam[6].toString().indexOf("SecureComponentRef") === -1, "Expected raw component in non-lockerized component.");
